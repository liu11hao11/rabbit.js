...
WorkerSocket.prototype.ack = function(info) {
    var msg,info_index;
    if(info){
      this.unacked.some(function (item,index) {
         if(item.content&&item.content.toString()==info){}
           msg=item;
          info_index=index;

       })
     this.unacked.splice(info_index,1);
    }else{
...
WorkerSocket.prototype.requeue = function(info) {
    var msg,info_index;
    if(info){
        this.unacked.some(function (item,index) {
            if(item.content&&item.content.toString()==info){}
...
WorkerSocket.prototype.discard = function(info) {
    var msg,info_index;
    if(info){
        this.unacked.some(function (item,index) {
            if(item.content&&item.content.toString()==info){}
